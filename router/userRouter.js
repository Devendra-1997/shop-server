...
import {
  ...
  insertUser,
  updateUser,
} from "../database/user/userModel.js";
import {
  loginUserValidator,
  newUserValidator,
...
import {
  ...
  findSession,
  insertSession,
} from "../database/session/sessionModel.js";
...
// signup
userRouter.post("/signup", newUserValidator, async (req, res, next) => {
  try {
    // hash password
...
// renew access
userRouter.get("/renew-access", jwtAuth, async (req, res, next) => {
  try {
...
// delete-account
userRouter.delete("/delete-account/:_id?", auth, async (req, res, next) => {
  try {
